...
function App() {
  ...
  const [settings, setSettings] = useState({
    temperature: 0.3,
    maxTokens: 2048,
    systemPrompt: `Convert the following text into concise, well-structured study notes.
...
8. Preserve all important terminology and concepts`
  ...
  const handleTextChange = (text) => {
    setInputText(text);
    // Clear any previous errors when text changes
  ...
  const handleGenerateNotes = async () => {
    ...
    try {
      const generatedNotes = await generateNotes(inputText, settings);
  ...
  return (
    <div className="app">
      ...
      {/* Settings button positioned in bottom left */}
